...
export default function EditFilmPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [filmResponse, genresResponse] = await Promise.all([
          api.getAdminFilm(id),
          api.getGenres(),
        ]);
        ...
        if (filmResponse.success && filmResponse.film) {
          const film = filmResponse.film;
          setTitle(film.title);
          setSlug(film.slug);
          setSynopsis(film.synopsis);
          setPosterUrl(film.poster_url || '');
          if (film.poster_url) setPosterPreview(film.poster_url);
          setTrailerUrl(film.trailer_url || '');
          setVideoUrl(film.video_url || '');
          setPrice(film.price.toString());
          setDurationMins(film.duration_mins?.toString() || '');
          setReleaseYear(film.release_year?.toString() || '');
          setRating(film.rating || '');
          setSelectedGenres(film.genres?.map((g) => g.id) || []);
          setIsPublished(film.is_published || false);
          setIsFeatured(film.is_featured || false);
        }
      } catch (err: any) {
        setError(err.response?.data?.message || 'Failed to load film');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Upload poster file first if a new file was selected
      let finalPosterUrl = posterUrl || null;
      if (posterFile) {
        setIsUploading(true);
        const uploadRes = await api.uploadPoster(posterFile, id);
        finalPosterUrl = uploadRes.url;
        setIsUploading(false);
      }

      const response = await api.updateFilm(id, {
        title,
        slug,
        synopsis,
      ...
      setError(err.response?.data?.message || 'Failed to update film');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-400 mt-1">Update film details</p>
        </div>
      </div>
